...
${formatInstructions}`;
  ...
  app.post("/api/admin/extract-deals", async (req, res) => {
    if (!req.session.isAdmin) {
      return res.status(401).json({ message: "Not authenticated as admin" });
    }
    try {
      const { extractDealsFromTranscript } = await import("./dealExtractor");
      const { ITUNES_ID_TO_SLUG } = await import("./podcastLandingMap");

      await storage.deleteAllDeals();

      const transcripts = await storage.getRecentTranscripts(100);
      ...
      for (const t of transcripts) {
        const key = `${t.podcastId}-${t.episodeGuid}`;
        if (processed.has(key)) continue;
        processed.add(key);

        try {
          const lookupUrl = `https://itunes.apple.com/lookup?id=${t.podcastId}&media=podcast`;
          const lookupRes = await fetch(lookupUrl);
          const lookupJson = await lookupRes.json();
          const podcastInfo = lookupJson.results?.[0];
          const podcastName = podcastInfo?.collectionName || "Unknown Podcast";

          const deals = await extractDealsFromTranscript({
            podcastId: t.podcastId,
            podcastName,
            episodeTitle: t.episodeTitle,
            episodeDate: t.fetchedAt ? new Date(t.fetchedAt).toLocaleDateString("en-US", { month: "long", day: "numeric", year: "numeric" }) : "Recent",
            transcript: t.transcript,
          });

          for (const deal of deals) {
            await storage.saveDeal(deal);
            totalDeals++;
          }

          res.write(JSON.stringify({ podcast: podcastName, episode: t.episodeTitle, dealsFound: deals.length }) + "\n");
        } catch (err) {
          res.write(JSON.stringify({ podcast: t.podcastId, episode: t.episodeTitle, error: String(err) }) + "\n");
        }
      }

      res.write(JSON.stringify({ done: true, totalDeals }) + "\n");
      res.end();
    } catch (err) {
      console.error("Error extracting deals:", err);
      res.status(500).json({ message: "Failed to extract deals" });
    }
  });
