...
async function main() {
  ...
  const totalIncomplete = podcastRows.reduce(
    (sum: number, r: any) => sum + parseInt(r.incomplete_count),
    0
  );
  console.log(
    "Processing " +
  ...
  for (let i = 0; i < podcastRows.length; i++) {
    ...
    try {
      const { rows: incompleteEps } = await pool.query(
        "SELECT id, episode_guid, episode_title FROM episode_transcripts WHERE podcast_id = $1 AND (complete_record IS NULL OR complete_record = false)",
        [p.itunes_id]
      );
      ...
      for (const e of incompleteEps) {
        if (e.episode_title)
          titleMap.set(e.episode_title.toLowerCase(), e.id);
      }
      ...
      for (let page = 1; page <= MAX_PAGES; page++) {
        ...
        const query =
          ...
          p.taddy_uuid +
          '") { uuid episodes(sortOrder: LATEST, limitPerPage: ' +
          EPISODES_PER_PAGE +
          ", page: " +
          page +
          ") { uuid name description subtitle datePublished duration imageUrl audioUrl seasonNumber episodeNumber episodeType } } }";
        ...
        for (const ep of episodes) {
          ...
          const matchByTitle =
            ep.name && titleMap.has(ep.name.toLowerCase());
          ...
          if (matchByGuid || matchByTitle) {
            ...
            try {
              await pool.query(
                "UPDATE episode_transcripts SET description = COALESCE($1, description), date_published = COALESCE($2, date_published), duration = COALESCE($3, duration), audio_url = COALESCE($4, audio_url), image_url = COALESCE($5, image_url), season_number = COALESCE($6, season_number), episode_number = COALESCE($7, episode_number), episode_type = COALESCE($8, episode_type), subtitle = COALESCE($9, subtitle), fetched_at = NOW(), complete_record = (transcript IS NOT NULL AND transcript != '' AND $13::boolean) WHERE podcast_id = $10 AND (episode_guid = $11 OR lower(episode_title) = lower($12))",
                [
                  ep.description,
                  ep.datePublished,
                  ep.duration,
      ...
      console.log("  Fixed " + fixed + "/" + incompleteEps.length);
      globalFixed += fixed;
    } catch (err: any) {
  ...
  const { rows: finalCount } = await pool.query(
    "SELECT COUNT(*) FILTER (WHERE complete_record = true) as complete, COUNT(*) FILTER (WHERE complete_record IS NULL OR complete_record = false) as still_incomplete FROM episode_transcripts"
  );
  ...
  console.log(
    "Fixed " +
      globalFixed +
      " total. Complete: " +
      finalCount[0].complete +
      ", Still incomplete: " +
      finalCount[0].still_incomplete
  );
  await pool.end();
}
