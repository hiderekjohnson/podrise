...
**Directory Caching**: Heavy directory endpoints (`/api/entities/people`, `/api/entities/companies`, `/api/bookstore`, `/api/podcasts-discovery`, `/api/podcasts/directory`) use a 24-hour in-memory cache via `DataCache` class. Caches auto-invalidate when new episode recaps are generated. Admin can manually refresh via the admin cache endpoint.
