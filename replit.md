...
- **CMS (Content Management System)**: Top-level admin tab with sub-navigation for Podcasts, Episodes, People, Companies, Products. Features include: searchable/sortable podcast and episode lists with status filtering, podcast detail with editable fields and auto-computed stats (episode count, recent guests, top topics), episode detail with editable recap fields (TLDL, whatHappened, reorderable takeaways), collapsible read-only transcript display, episode quotes CRUD from `episode_quotes` table, featured quote editing, guest/sponsor/book/tool display, status management (published/needs_review/hidden), and AI regeneration via existing endpoint. Both `landing_page_recaps` and `podcast_directory` tables have a `status` text column (migrated from boolean `published`). CMS component: `client/src/pages/AdminCMS.tsx`. API endpoints: `GET/PATCH /api/admin/cms/podcasts`, `GET/PATCH /api/admin/cms/episodes`, quote CRUD at `/api/admin/cms/episodes/:slug/:slug/quotes` and `/api/admin/cms/quotes/:id`.
- **Entity Management**: Proper database tables for people (`entity_people`) and companies (`entity_companies`) with full profile fields (bio, photo, social links, category, verified status). Junction table `entity_episode_mentions` links entities to episodes with context snippets. Entity detection runs via text-matching against transcripts (free, no AI). Backfill populates both `entity_contexts_cache` on recaps and the junction table. API: `GET/POST/PATCH /api/admin/cms/people`, `GET/POST/PATCH /api/admin/cms/companies`, `GET /api/admin/cms/entity-mentions/:type/:slug`. CMS People/Companies tabs show clickable entity cards with drill-down detail panels showing editable profiles and episode mention lists.
