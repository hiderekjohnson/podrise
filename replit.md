...
**Feature Usage Analytics**: A "Features" sub-tab in Admin > Internal Tools > Analytics provides product-level insight into how users engage with each major in-app feature. Five panels: AI Chat (questions asked, unique users, top episodes, per-user breakdown), Audio Recaps (total plays, unique listeners, completion rate ≥80%, top episodes), Pod Shop (affiliate link clicks trend, unique users, top products), Spotify Import (connected users, % of all users, daily import sessions, avg shows/import), and Listen to Episode (clicks by platform, daily trend, top episodes). Data is sourced from: `feature_events` table (ai_chat, episode_link, spotify_import events), `audio_playback_events` (existing), `affiliate_clicks` (now includes `user_id`). Instrumentation: episode-chat API logs ai_chat events; EpisodePageLayout listen links log episode_link events on click; SettingsPage Spotify bulkFollow logs spotify_import events after success. New endpoint: `POST /api/feature-events` for client-side event tracking. Admin endpoints: `/api/admin/analytics/features/ai-chat`, `/audio`, `/shop`, `/spotify`, `/episode-links`.
**Episode Processing Pipeline** (`server/productionRecapScheduler.ts`): Observable, one-at-a-time sequential pipeline. Taddy webhooks ONLY queue episodes (never process inline). Two independent loops:
1. **Transcript Fetcher** (every 90s): Picks ONE `queued` episode, fetches transcript from Taddy, moves to `transcript_ready`. Max 5 attempts before `failed`.
2. **Recap Generator** (every 5min): Picks ONE `transcript_ready` episode, generates AI recap, moves to `completed`. Includes headline generation and validation.
Pipeline stages: `queued` → `fetching` → `transcript_ready` → `generating_recap` → `completed` / `failed`. All stages tracked in `pending_transcript_queue` table. Admin pipeline page shows real-time status with countdown timers for next fetch/generation.
Supporting systems:
- **Active Catch-Up Scanner** (every 2h): Polls Taddy for missed episodes, queues them into the pipeline (not processed directly).
- **Recap Pipeline Health Monitor** (every 30min): Alerts on stalls.
- **Headline Catch-Up** (every 15min): Fills missing tabloid headlines.
**Data Backfills**: A "Backfill" sub-tab under Admin > Advanced provides a structured way to run and monitor one-off data backfill jobs. Each job is displayed as a card with description, rate note, created date, last run date, and last run results. While a job is running, a live progress bar polls `/api/admin/backfills/:key` every 3 seconds. Progress persists across page reloads. First backfill: "Backfill Tabloid Headlines" — generates `tabloidHeadline`/`tabloidSubHeadline` for all `landing_page_recaps` rows missing them, one every 10 seconds. DB table: `backfill_jobs` (key, status, total_records, processed_count, updated_count, error_message, last_run_at). API: `GET /api/admin/backfills`, `GET /api/admin/backfills/:key`, `POST /api/admin/backfills/:key/run`. On server restart, any "running" jobs are marked "failed/interrupted".
