...
**Podcast Bookstore**: Features routes for a discovery grid (`/bookstore`) and individual book pages (`/bookstore/:bookSlug`). Book data is extracted from AI-generated recaps and enriched with descriptions, podcast buzz, and Amazon affiliate links. Book covers are sourced from Amazon or Open Library API.
**Episode Recap Page Sections**: Structured content flow including SEO intro, Key Takeaways, Notable Quotes, Full Recap, Guests, Notable Mentions, Key Topics, Books Mentioned, Sponsors, Hosts, Questions Answered, and Podcast Chat.
