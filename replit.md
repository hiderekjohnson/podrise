...
- **Admin Tools**: A dashboard for user management, episode generation status, podcast expansion, data backfilling, and cache management. Includes a comprehensive analytics hub with four sub-dashboards: User Acquisition, Affiliate Performance, User Growth, and Email Marketing — each with time-range filtering and granularity controls. An "API Costs" tab tracks OpenAI API usage with daily/weekly/monthly spend summaries, a Recharts daily bar chart, budget progress bar, feature breakdown, model breakdown, and recap-specific metrics. A "Shop Mgmt" tab provides a unified view of all shop items (books and products) with search, type/status filters, and an edit modal for updating name, description, URL, and image (with file upload). Image upload supports JPG/PNG/WebP with 10MB limit, stores to `public/uploads/` (products) or `public/books/` (books), and shows retina dimension guidance. An "Admins" tab manages admin users (`admin_users` table) with full CRUD — add new admins with email/name/role (owner or admin), edit details, and delete with confirmation.
