...
**Recap Pipeline Reliability System** (`server/productionRecapScheduler.ts`): Three-layer protection against recap gaps:
1. **Active Catch-Up Scanner** (every 2h): Proactively polls Taddy's API for every published podcast that hasn't received a transcript in 3+ hours. Fetches and saves any missing transcripts directly, bypassing webhook dependency. Caps at 60 podcasts/run. Immediately triggers a recap batch after any recovery. Runs first at server startup + 10min.
2. **Recap Pipeline Health Monitor** (every 30min): Two alert modes — CRITICAL (transcripts in DB 30+ minutes with no recap generated, scheduler is broken) and WARNING (no new recaps in 5+ hours during 8am–11pm UTC, Taddy delay likely). Resets automatically when next recap is generated.
3. **NPR News Now Monitor** (every 30min): Alerts if NPR News Now (hourly publisher) has no new transcript in 90 minutes. 2-hour grace period after approval.
