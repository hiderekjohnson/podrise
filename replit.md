...
## Podcast Bookstore
**Routes**: `/bookstore` (discovery grid) and `/bookstore/:bookSlug` (individual book pages).
**API**: `GET /api/bookstore` returns `{ books: [...], total: number }` for the grid. `GET /api/bookstore/:bookSlug` returns full book detail with episodes, related books, and metadata.
...
**Enrichment**: `book_enrichments` table stores AI-generated descriptions, podcast buzz, ASINs, and URL slugs. Enrichment via `POST /api/admin/enrich-books` or `server/enrichBooks.ts`. Slugs auto-generated on enrichment (format: `title-author`, lowercase, hyphens).
**Two-page strategy**: Bookstore grid is the discovery engine (enriched books link internally to detail pages via "See All Mentions"). Individual book pages are conversion pages with Amazon affiliate CTAs (tag: `podcap-20`). Non-enriched books link directly to Amazon from the grid.
**Individual book pages**: Hero (cover, title, author, stats), "About This Book", "What Podcasters Are Saying" (buzz), "Episodes Featuring [book]" (linked to recap pages), "Listeners Also Read" (related books from same episodes), dual Amazon CTA, affiliate disclosure. Full SEO (title, meta, canonical, OG).
**Book Covers**: Amazon image via ASIN (primary), Open Library API fallback (search by title → cover_i ID).
**Affiliate Disclosure**: Shown at bottom of pages per FTC guidelines.
