...
**Book Covers**: 209 local covers stored in `public/books/{slug}.jpg`. All BookCover components across pages (EpisodeRecapPage, BookDetailPage, Bookstore, PodcastLandingGeneric, PersonDetailPage) use local-first loading: local file -> Amazon CDN -> Open Library -> placeholder. State resets on prop changes to prevent stale images.
