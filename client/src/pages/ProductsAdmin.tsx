...
import { Loader2, ExternalLink, ShoppingBag, Play, Package, Radio, FileText, Star, MessageSquare, ThumbsUp, ThumbsDown, Check, X, Filter, Clock, CheckCircle2, XCircle } from "lucide-react";
...
export default function ProductsAdmin() {
  ...
  const approveMutation = useMutation({
    ...
    onSuccess: (_, ids) => {
      toast({ title: "Approved", description: `${ids.length} product(s) approved — AI will learn from this` });
  ...
  const rejectMutation = useMutation({
    ...
    onSuccess: (_, { ids }) => {
      toast({ title: "Rejected", description: `${ids.length} product(s) rejected — AI will avoid similar ones` });
      setRejectingId(null);
      queryClient.invalidateQueries({ queryKey: ["/api/admin/products"] });
    },
  });
    ...
    <div className="space-y-4">
      ...
      {lastExtraction && !extracting && (
        <div className="p-3 rounded-xl bg-green-50 border border-green-200 text-green-700 text-sm flex items-center gap-3" data-testid="text-extraction-result">
          ...
          <span>Found {lastExtraction.newCount} new products ({lastExtraction.coverage} transcript coverage)</span>
        </div>
      )}
      ...
      {isLoading ? (
        <div className="text-center py-10 text-muted-foreground text-sm">Loading products...</div>
      ) : products.length === 0 ? (
        <div className="text-center py-10">
          <ShoppingBag className="w-12 h-12 text-muted-foreground/30 mx-auto mb-3" />
          <p className="text-sm text-muted-foreground">
            {filter === "pending" ? "No products waiting for review. Run extraction to find new ones." :
             filter === "approved" ? "No approved products yet. Review pending products to build the training set." :
             filter === "rejected" ? "No rejected products yet." :
             "No products extracted yet. Click 'Extract New Products' to scan episodes."}
          </p>
        </div>
      ) : (
