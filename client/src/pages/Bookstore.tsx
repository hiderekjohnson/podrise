  ...
  return (
    ...
    >
      <Link href={`/shop/${product.slug}`} className="block">
        <div className="p-5">
          <div className="flex gap-4">
            <div className={`w-[100px] h-[100px] rounded-xl shrink-0 overflow-hidden flex items-center justify-center ${hasImage ? "" : "bg-[#FAFAFA] dark:bg-white/[0.02]"}`}>
              {hasImage ? (
        ...
        {products.slice(0, 10).map((product, i) => {
          ...
          return (
            <Link href={`/shop/${product.slug}`} className="block shrink-0" key={`${keyPrefix}-${product.name}-${i}`} data-testid={`shelf-${keyPrefix}-${product.slug}-${i}`}>
              <div className="w-[160px] shrink-0 group/shelf">
                <div className={`w-[160px] h-[160px] rounded-xl mb-2.5 overflow-hidden flex items-center justify-center ${hasImage ? "bg-white dark:bg-white/[0.03]" : "bg-[#FAFAFA] dark:bg-white/[0.02]"}`}>
                  {hasImage ? (
                    <img src={product.imageUrl!} alt={product.name} className="w-full h-full object-cover" loading="lazy" onError={() => setImgErrors(prev => new Set(prev).add(i))} />
                  ) : (
