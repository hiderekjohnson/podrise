...
function BookCard({ book, index }: { book: BookstoreBook; index: number }) {
  ...
  const coverEl = <BookCover title={book.name} asin={book.asin} author={book.author} size="md" />;
  const titleEl = (
  ...
  return (
    ...
    >
      <div className="p-5">
        ...
        {book.description && (
          <p className="text-sm text-muted-foreground leading-relaxed mt-4 line-clamp-3" data-testid={`book-description-${index}`}>
            {book.description}
          </p>
        )}
